...
public class MenuScript
{
	...
	void InGameUI()
	{
		...
		Rect healthRect = new Rect(hpmpBG.x+hpmpBG.width*0.005f, hpmpBG.y+hpmpBG.height/2*0.15f, 100, 26);
		...
		//mp: 2/320 by 40/72
		//maths
		Rect displayHealth = healthRect;
		displayHealth.width = healthRect.width * (PlayerInfo.currentHealth*1.0f/PlayerInfo.healthLimit*1.0f);

		//gui
		GUI.DrawTexture(hpmpBG, TomScript.thisInstance.artAssets[1]);
		GUI.DrawTexture(trBG, TomScript.thisInstance.artAssets[2]);
		GUI.DrawTexture(displayHealth, TomScript.thisInstance.artAssets[0]);
